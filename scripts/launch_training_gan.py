...
def _build_arg_parser():
    ...
    t.add_argument('--learning_rate_g', nargs='+', default=[0.0001], type=float,
                   help='List of learning rate  to try [%(default)s].')
    t.add_argument('--learning_rate_d', nargs='+', default=[0.0001], type=float,
                   help='List of learning rate  to try [%(default)s].')
    t.add_argument('--weight_decay_g', nargs='+', default=[0.005], type=float,
                   help='List of learning rate  to try [%(default)s].')
    t.add_argument('--weight_decay_d', nargs='+', default=[0.005], type=float,
                   help='List of learning rate  to try [%(default)s].')
...
def main():
    ...
    if not args.evaluate_test_set:
        ...
        config = {
            ...
            "lr_g": tune.grid_search(args.learning_rate_g),
            "lr_d": tune.grid_search(args.learning_rate_d),
            "batch_size": tune.grid_search(args.batch_size),
            "wd_g": tune.grid_search(args.weight_decay_g),
            "wd_d": tune.grid_search(args.weight_decay_d),
